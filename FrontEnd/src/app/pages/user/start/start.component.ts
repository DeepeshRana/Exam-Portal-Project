...
export class StartComponent implements OnInit {
  ...
  evalQuiz() {
    ...
    const payload = this.questions.map(question => ({
      ...
      quiz: question.quiz ? { qId: question.quiz.qId } : null // Add null check
    }));
  
    // Log the payload to inspect its structure
    console.log('Payload:', JSON.stringify(payload));
  
    this._question.evalQuiz(payload).subscribe(
      (data: any) => {
        console.log('Response data:', data);
  
        if (data && data.marksGot !== undefined && data.correctAnswers !== undefined && data.attempted !== undefined) {
          this.marksGot = data.marksGot;
          this.correctAnswers = data.correctAnswers;
          this.attempted = data.attempted;
        } else {
